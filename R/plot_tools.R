...
cumulative_plot = function(df_aggregated, plot_date, col) {
  plot_df = subset(df_aggregated, date<=plot_date)

  g1 = ggplot(plot_df, aes(x = date, y = cases, color = "Global")) +
    geom_line() +
...
new_cases_plot = function(df_aggregated, plot_date, col) {
  plot_df_new = subset(df_aggregated, date<=plot_date)

  g1 = ggplot(plot_df_new, aes(x = date, y = new_cases, fill = "Global")) +
